...
There are lots of frameworks for building dashboards; one of the simplest works directly in Rmarkdown. Here's an example (taken from <https://bookdown.org/yihui/rmarkdown/shiny.html#a-shiny-dashboard-example>):

``` markdown
`r xfun::file_string('https://raw.githubusercontent.com/rstudio/rmarkdown-book/main/examples/dashboard/07-shiny.Rmd')`
```

# Your turn!

Let's try it! Modify the following code (from the linear regression class) into an rmarkdown file with shiny buttons for `n`, `intercept` and `slope`.
